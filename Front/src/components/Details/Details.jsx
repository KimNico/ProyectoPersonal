...
import { getPublicacionesById } from './path/to/your/actions'; // Ajusta el path según corresponda
import styles from './Details.module.css'; 

export const Details = () => {
  const { id_trabajo } = useParams();
  const dispatch = useDispatch();
  const job = useSelector(state => state.publicaciones.find(job => job.id === parseInt(id_trabajo)));

  useEffect(() => {
    dispatch(getPublicacionesById(id_trabajo));
  }, [dispatch, id_trabajo]);
